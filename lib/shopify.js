...
async function createOrder(p) {
  ...
  if (p.email) {
    // Link the customer by email. Only attach names when BOTH are present:
    // Shopify rejects a blank customer.last_name (422), which broke orders
    // for shoppers who entered a single-word name on Stripe. The shopper's
    // name still appears on the order via the shipping/billing address.
    order.customer = { email: p.email };
    const fn = ((p.customer && p.customer.first_name) || '').trim();
    const ln = ((p.customer && p.customer.last_name) || '').trim();
    if (fn && ln) {
      order.customer.first_name = fn;
      order.customer.last_name = ln;
    }
  }
